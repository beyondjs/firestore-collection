...
export /*bundle*/ class Collection<DataType> {
	...
	col(params?: { parents?: Record<string, string> }): CollectionReference<DataType> {
		...
		if (!this.#parent) return <CollectionReference<DataType>>collection(db, this.#name);

		// Retrieve the parent document reference
		const parentCollection = this.#parent.col({ parents });
		const docId = parents[this.#parent.#name];
		const parentDoc = doc(parentCollection, docId);

		// Reference the subcollection
		return collection(parentDoc, this.#name) as CollectionReference<DataType>;
	}
	...
	doc(params: { id: string; parents?: Record<string, string> }): DocumentReference<DataType> {
		...
		// Get the collection reference using the 'col' method
		const collection = this.col({ parents });

		// Return the document reference using the 'doc' function
	 ...
	 * - If `transaction` is not provided, it simply fetches the document using `getDoc(docRef)`.
	 *
	...
	async snapshot(params: {
		...
		try {
			// Fetch the document snapshot, using the transaction if provided
			const doc = this.doc(params);
			const snapshot = await (transaction ? transaction.get(doc) : await getDoc(doc));
	...
	async set(params: {
		...
		const doc: DocumentReference<DataType> = this.doc({ id, parents });

		try {
			await (transaction ? transaction.set(doc, data) : setDoc(doc, data));
	...
	async merge(params: {
		...
		try {
			await (transaction ? transaction.set(doc, data, { merge: true }) : setDoc(doc, data, { merge: true }));
			return new Response({ data: { stored: true } });
		} catch (exc) {
			const error = ErrorGenerator.documentNotSaved(this.#name, id);
			return new Response({ error });
	...
	async delete(params: {
		...
		try {
			await (transaction ? transaction.delete(doc) : deleteDoc(doc));
